...
def projD(array, ax):
    ...
    norms = np.maximum(1, np.sqrt(np.sum(np.absolute(npA)**2, axis = ax))) # i am assumig we are changing only the ones which are not the unit ball
...
def algorithm1(hbar_tensor, N, ax, Tensor):
    """
    ...
    for _ in range(N):
        ...
        k_t = k_t + lambdat*(projD((2*h_t - k_t + hbar)/2, ax) - h_t) # this won't be affected if i do not do the projection onto C although change code eventually
